...
namespace BeeGees_ReadNode
{
    public class ReaderEntryPoint
    {
        ...
        {
            Log.Info($" [x] Received event (Type: {(MessageType)BaseMessage.Parser.ParseFrom(e.Body.ToArray()).Type}, CID: {e.BasicProperties.CorrelationId})");
            ...
            if (response != null)
            {
                Log.Info($" [x] Sending confirmation to write node (CID: {e.BasicProperties.CorrelationId})");
        ...
        private async Task Consumer_ReceivedAsync(object sender, BasicDeliverEventArgs e)
        {
            ...
            Log.Info($" [x] Received query (Type: {(MessageType)message.Type}, CID: {e.BasicProperties.CorrelationId})");
            ...
            if (response != null)
            {
                Log.Debug($" [x] Sending query response to client (CID: {e.BasicProperties.CorrelationId})");
